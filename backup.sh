...
sshpass -p "$PASS" ssh -q -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no  -i /certs/$PRIV_KEY_NAME  $DISTANT_HOST_PATH "ls -1trpd $BACKUP_PATH/* | head -n -$KEEP | xargs -d '\n' rm -f --"
fi
...
ls -1trpd /save/* | head -n -$KEEP | xargs -d '\n' rm -f --
